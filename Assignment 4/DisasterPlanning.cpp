 ...
 */
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,int& numCities,Set<string>&uncovered
                               ,Set<string>& supplyLocations){
    if(uncovered.isEmpty()&&numCities>=0){
        return true;
    }
    if(numCities<0){
        return false;
    ...
    auto uncovered_temp=uncovered;
    auto supply_temp=supplyLocations;
    supply_temp+=first;
    uncovered_temp.remove(first);
    for(auto f:roadNetwork[first]){
        uncovered_temp.remove(f);
    }
    for(auto u:uncovered_temp){

    }
    if(canBeMadeDisasterReadyRec(roadNetwork,--numCities,uncovered_temp,supply_temp)){
        supplyLocations=supply_temp;
        return true;
    }
    else{
        numCities++;
    }
    for(auto second:roadNetwork[first]){
        auto uncovered_temp=uncovered;
        auto supply_temp=supplyLocations;
        supply_temp+=second;
        uncovered_temp=uncovered_temp-second-roadNetwork[second];
        for(auto u:uncovered_temp){
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities<0){
        error("the city number should greater than 0");
    }

    Set<string>uncovered;
    Vector<string>r=roadNetwork.keys();
    for(auto a:r){
        uncovered+=a;
    }
...
//        return true;
//    }
    return canBeMadeDisasterReadyRec(roadNetwork, numCities,uncovered,supplyLocations);
