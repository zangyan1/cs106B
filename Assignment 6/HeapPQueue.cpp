...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    /* TODO: Delete the next line and implement this. */
   //for test
    auto test1=data.name;
    auto test2=logicalSize;
    auto test3=allocatedSize;
    if(size()>=allocatedSize-1){ //要减1吗？？？？？？？？？？？
        //开辟空间
           d=new DataPoint[2*HeapPQueue::INITIAL_SIZE];
           memcpy(d,elems,allocatedSize);
